...
#################################################################################################################################
def extract_features(url, status = -1):
    ...
    if state:
        ...
        domain_age, domain_registration_length = trdfe.domain_info(domain_age_key, domain)
        row = [
                url,
                # # # url-based features
...
def is_URL_accessible(url):
    ...
    def fetch_page(url):
        ...
        try:
            ...
            parsed = urlparse(url)            
            if not parsed.scheme:  # Nếu không có scheme, thêm 'http://' vào trước URL
                url = 'http://' + url
                parsed = urlparse(url)
            if not parsed.netloc.startswith('www'):  # Nếu không có www, thêm vào trước domain
